...
[role="lead"]
NetApp Backup and Recoveryでは、オンプレミスのONTAPシステムからセカンダリ システムおよびAzure Blobにボリューム データをバックアップできます。このページを使用して環境を準備し、バックアップを有効にします。
...
+
NetApp Backup and Recovery は、コンソール エージェントがクラウドにデプロイされている場合に Azure Government リージョンでサポートされます。オンプレミス環境にインストールされている場合はサポートされません。さらに、Azure Marketplace からコンソール エージェントをデプロイする必要があります。NetApp Console SaaS ウェブサイトから Government リージョンにコンソール エージェントをデプロイすることはできません。
...
.開始する前に
Azure Synapse Analytics リソース プロバイダー（「Microsoft.Synapse」という名前）をサブスクリプションに登録する必要があります。 https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/resource-providers-and-types#register-resource-provider["このリソースプロバイダーをサブスクリプションに登録する方法をご覧ください"^]。リソース プロバイダーを登録するには、サブスクリプションの *Owner* または *Contributor* である必要があります。
...
+
** NetApp Backup and Recovery の PAYGO ライセンスには、 https://marketplace.microsoft.com/en-us/product/netapp.cloud-manager?tab=Overview["Azure Marketplace からのNetApp Consoleの提供"^]へのサブスクリプションが必要です。NetApp Backup and Recovery の請求は、このサブスクリプションを通じて行われます。
...
* デフォルトとは異なるIPspaceを使用している場合は、オブジェクトストレージにアクセスするために静的ルートを作成する必要があるかもしれません。
...
+
NetApp Backup and Recoveryは、アクセス許可モデルとして「Azureアクセス ポリシー」をサポートしています。「Azureロールベース アクセス制御」（Azure RBAC）アクセス許可モデルは現在サポートされていません。
...
+
** Console の「Systems」ページからシステムを選択し、右側のパネルにある「Backup and recovery」サービスの横にある「Enable」>「Backup Volumes」を選択します。
+
...
+
ウィザードの「はじめに」ページには、ローカルSnapshot、レプリケーション、バックアップなどの保護オプションが表示されます。この手順で2番目のオプションを選択した場合、「バックアップ戦略の定義」ページが表示され、1つのボリュームが選択された状態になります。
...
====
.手順
選択したボリュームに既にスナップショットまたはレプリケーションのポリシーが適用されている場合、後で選択したポリシーによって既存のポリシーが上書きされます。
...
NOTE: *独自のバケットを使用する* を選択した場合は、バックアップ先として作成するコンテナーの名前が Azure ストレージ アカウントと同じであることを確認してください。
...
*** ポリシーの名前を入力します。
*** 通常は異なる頻度のスケジュールを最大 5 つ選択します。
*** オブジェクトへのバックアップ ポリシーについては、DataLock とランサムウェア対策の設定を行います。DataLock とランサムウェア対策の詳細については、link:prev-ontap-policy-object-options.html["オブジェクトへのバックアップポリシー設定"] を参照してください。
*** *作成*を選択します。
...
.手順
. 「レビュー」ページで選択内容を確認します。
. オプションで、*スナップショット ポリシー ラベルをレプリケーションおよびバックアップ ポリシー ラベルと自動的に同期する*チェックボックスをオンにします。これにより、レプリケーション ポリシーおよびバックアップ ポリシーのラベルと一致するラベルを持つスナップショットが作成されます。
. *バックアップの有効化*を選択します。
