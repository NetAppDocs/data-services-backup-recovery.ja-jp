...
---
= Kubernetes アプリケーションを追加して NetApp Backup and Recovery で保護する
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
NetApp Backup and Recovery の Web UI またはカスタムリソースファイルを使用して、Kubernetes アプリケーションを保護対象として追加します。標準の Kubernetes リソースを使用する名前空間ベースのアプリケーション、または1つ以上の仮想マシンを使用する VM ベースのアプリケーションを追加できます。

*必須 NetApp Console ロール*

Backup and Recovery スーパー管理者、または Backup and Recovery バックアップ管理者。link:reference-roles.html["NetApp Backup and Recoveryのアクセス ロールについて学習します"]. https://docs.netapp.com/us-en/console-setup-admin/reference-iam-predefined-roles.html["すべてのサービスに対するNetApp Consoleのアクセスロールについて学習します"^].
...
Kubernetes アプリケーションを保護するには、まず NetApp Backup and Recovery でアプリケーションを作成します。アプリケーションを作成すると、NetApp Backup and Recovery は Kubernetes クラスタ上で実行されているアプリケーションを認識します。
...
.結果
アプリケーションが作成され、Kubernetesインベントリの「アプリケーション」タブにあるアプリケーション一覧に表示されます。NetApp Backup and Recoveryは、お客様の設定に基づいてアプリケーションの保護を有効にします。進行状況は「監視」エリアで確認できます。
