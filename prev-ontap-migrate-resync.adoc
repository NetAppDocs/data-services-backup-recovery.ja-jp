...
[role="lead"]
SnapMirror to Cloud Resync を NetApp Backup and Recovery で使用すると、SnapMirror 論理レプリケーション（LRSE）でボリュームを移行した後も、既存のクラウドバックアップを引き続き利用できます。この機能により、再ベースラインなしでバックアップを継続でき、ONTAP 9.16.1 以降で利用可能です。

SnapMirror to Cloud Resync は、FlexVol および FlexGroup の移行をサポートし、ハイブリッドおよびマルチクラウド環境全体でバックアップの継続性を維持するのに役立ちます。
...
オンプレミスのボリューム Vol1a があるとします。このボリュームには、S1、S2、S3 の 3 つの Snapshot があります。これらの Snapshot はリストア ポイントです。Vol1a は SnapMirror to Cloud （SM-C） を使用してクラウドにバックアップされていますが、オブジェクト ストアには S1 と S2 のみが格納されています。
...
NOTE: 古い ONTAP クラスタから新しい ONTAP クラスタへのボリュームのクローン作成、またはカスケードアーキテクチャ設定では、「Vault」タイプの SnapMirror ポリシーを使用しないでください。Vault ポリシーは、転送中にスナップショットのバージョン UUID を変更します。スナップショットのバージョン UUID は、ソースとデスティネーションの間に共通のスナップショットが存在するかどうかを判断するために使用されます。
...
+
NetApp Backup and Recoveryは、再同期のために選択された各ボリュームが、オブジェクトストアと共通のスナップショットを共有していることを検証します。具体的には、オブジェクトストアに保存されている最新のスナップショットは、新しくクローンされたボリュームのローカルスナップショットにも存在している必要があります。このチェックにより、ボリュームがSnapMirror to Cloud再同期操作の準備ができていることが保証されます。

. 新しいソース ボリューム名や各ボリュームの再同期ステータスなどの検証結果を確認します。
. ボリュームの適用条件を確認してください。システムは、ボリュームが再同期の対象であるかどうかを確認します。ボリュームが対象外となる場合、共通Snapshotの基準を満たしていないことを意味します。
+
