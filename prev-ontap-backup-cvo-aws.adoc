...
---
= NetApp Backup and Recoveryを使用してCloud Volumes ONTAPをAmazon S3にバックアップする
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
NetApp Backup and Recovery を使用して、Cloud Volumes ONTAP システムのボリュームデータを Amazon S3 にバックアップします。
...
NOTE: バックアップまたは復元データの転送にプライベートVPCエンドポイントを使用する場合、設定されているプロキシ接続よりも優先されます。
...
バックアップを保存するストレージスペースには、AWSアカウントも必要です。
...
必要なAWS Cloud Volumes ONTAP権限:: Cloud Volumes ONTAPシステムがONTAP 9.12.1以降のソフトウェアを実行している場合、そのシステムに権限を提供するIAMロールには、最新のNetApp Backup and Recovery専用の新しいS3権限セットが含まれている必要があります。 https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-set-up-iam-roles.html["Cloud Volumes ONTAPポリシー"^] 。
+
--
...
新規または既存の Cloud Volumes ONTAP システムで NetApp Backup and Recovery を有効にすることができます。手順はオプションごとに若干異なります。
...
.手順
. コンソールの「Systems」ページからクラスターを選択し、右側のパネルにある「Backup and Recovery」の横にある「Enable」を選択します。
+
バックアップのAmazon S3の保存先が*システム*ページにシステムとして存在する場合は、クラスターをAmazon S3システムにドラッグしてセットアップ ウィザードを開始できます。
...
Cloud Volumes ONTAP システムからいつでも直接バックアップを有効化できます。
...
.手順
. 以下のいずれかの方法で、バックアップと復元の有効化ウィザードにアクセスしてください。
+
** Console の「Systems」ページからシステムを選択し、右側のパネルにある「Backup and Recovery」の横にある「Enable」>「Backup Volumes」を選択します。
+
バックアップの Amazon S3 の保存先が、コンソールの *システム* ページにシステムとして存在する場合は、ONTAP クラスターを Amazon S3 システムにドラッグできます。

** Backup and Recovery バーで「ボリューム」を選択します。「ボリューム」タブから「アクション」image:icon-action.png["アクションアイコン"]アイコンを選択し、単一ボリューム（既にレプリケーションまたはオブジェクトストレージへのバックアップが有効になっていないボリューム）に対して「3-2-1保護を有効にする」を選択します。


+
ウィザードの「はじめに」ページには、ローカルスナップショット、レプリケーション、バックアップなどの保護オプションが表示されます。この手順で2番目のオプションを選択した場合、「バックアップ戦略の定義」ページが表示され、1つのボリュームが選択された状態になります。
...
+
お客様ご自身で管理するキーを使用する場合は、キー情報を入力してください。
...
.手順
. 「レビュー」ページで選択内容を確認します。
. 必要に応じて、*ローカルスナップショット、レプリケーション、バックアップ上の不一致ラベルを自動的に修正する* のチェックボックスをオンにしてください。これにより、スナップショット、レプリケーション、およびバックアップ ポリシー内のラベルと一致するラベルを持つスナップショットが作成されます。
. *バックアップの有効化*を選択します。
...
バックアップとリカバリの有効化ウィザードで使用されるAPIコマンドを表示し、必要に応じてコピーすることができます。将来のシステムでバックアップの有効化を自動化するために、この操作を行うことをご検討ください。

.手順
. バックアップとリカバリの有効化ウィザードから、*APIリクエストの表示*を選択します。
