...
---
= SnapCenter の NetApp Backup and Recovery へのインポートの前提条件
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
SnapCenter Plug-in for Microsoft SQL Server から NetApp Backup and Recovery にリソースをインポートする前に、環境がこれらの追加要件を満たしていることを確認してください。
...
== SnapCenterプラグインのホスト要件を満たしていることを確認します

Microsoft SQL Server 用 SnapCenter プラグインからリソースをインポートするには、Microsoft SQL Server 用 SnapCenter プラグインをインストールするためのホスト要件が満たされていることを確認してください。
...
** `1`：UACのリモート制限は無効になっています。ローカルアカウントはトークンフィルタリングをバイパスし、リモートアクセス時に完全な管理者権限を持ちます。
...
----
. 次のコマンドを実行して、 `LocalAccountTokenFilterPolicy`で現在の値が「1」に設定されていることを確認します：
+
[listing]
----
