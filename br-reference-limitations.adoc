...
** SnapMirrorデスティネーションボリュームにローカルのSnapshotコピーを作成した場合（使用されているSnapMirrorラベルに関係なく）、これらのSnapshotはバックアップとしてクラウドに移動されません。NetApp Backup and Recoveryでバックアップできるようにするには、ソースDPボリュームに目的のラベルを付けたSnapshotポリシーを作成する必要があります。
