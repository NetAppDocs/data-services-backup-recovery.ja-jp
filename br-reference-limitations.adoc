...
[role="lead"]
NetApp Backup and RecoveryをONTAPボリュームに使用する前に、これらの制限事項を確認してください。
...
* レプリケーションのために一度に選択できるFlexGroupボリュームは1つだけです。各FlexGroupボリュームのバックアップを個別に有効化する必要があります。
+
FlexVolボリュームにはそのような制限はありません。システム内のすべてのFlexVolボリュームを選択して、同じバックアップポリシーを割り当てることができます。
...
+
** SnapMirrorラベル `app_consistent`および `all_source_snapshot`との関係は、クラウドにバックアップされません。
** SnapMirrorデスティネーションボリューム上にスナップショットのローカルコピーを作成した場合（使用されているSnapMirrorラベルに関係なく）、これらのスナップショットはバックアップとしてクラウドに移動されません。NetApp Backup and Recoveryでバックアップできるようにするには、移行元のDPボリュームに必要なラベルを指定したスナップショットポリシーを作成する必要があります。


* FlexGroupボリュームバックアップはアーカイブストレージに移動できません。
...
** ボリュームに適用されるスナップショット ポリシーは、NetApp Backup and Recoveryで認識されるポリシー（日次、週次、月次など）のいずれかでなければなりません。デフォルトの「sm_created」ポリシー（「すべてのスナップショットをミラーリング」に使用）は認識されず、DPボリュームはバックアップ可能なボリュームのリストに表示されません。
** SVM-DRとボリュームのバックアップおよびリカバリは、バックアップがソースまたは宛先のどちらから取得された場合でも、完全に独立して動作します。唯一の制限は、SVM-DRがSnapMirrorクラウドリレーションシップをレプリケートしないことです。DRシナリオでストレージVMがセカンダリロケーションでオンラインになった場合、SnapMirrorクラウドリレーションシップを手動で更新する必要があります。


* MetroClusterサポート:
+
** ONTAP 9.12.1 GA以降を使用する場合、プライマリシステムに接続されているときにバックアップがサポートされます。バックアップ構成全体がセカンダリ システムに転送されるため、スイッチオーバー後もクラウドへのバックアップが自動的に継続されます。セカンダリ システムでバックアップを設定する必要はありません（実際には、設定することは制限されています）。
...
* 「今すぐバックアップ」ボタンを使用したアドホックなボリュームバックアップは、データ保護ボリュームではサポートされていません。
* SM-BC 構成はサポートされていません。
* ONTAPは、単一ボリュームから複数のオブジェクトストアへのSnapMirror関係のファンアウトをサポートしていません。そのため、この構成はNetApp Backup and Recoveryではサポートされていません。
