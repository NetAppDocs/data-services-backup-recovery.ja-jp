...
[role="lead"]
この情報を使用して、NetApp Backup and Recovery バックアップ用の AWS S3 アーカイブ ティアを選択してください。アーカイブされたバックアップのリストアコストと取得時間も確認できます。
...
バックアップファイルが最初に作成されると、S3 _Standard_ ストレージに保存されます。この階層は、アクセス頻度の低いデータの保存に最適化されていますが、即時アクセスも可能です。30日後、バックアップデータはコスト削減のため、S3 _Standard-Infrequent Access_ ストレージクラスに移行されます。
