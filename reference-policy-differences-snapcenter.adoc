...
---
= SnapCenterとNetApp Backup and Recoveryのポリシーの違い
:hardbreaks:
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
[role="lead"]
この資料を参照して、インポートされたリソースやポリシーに影響する可能性がある SnapCenter と NetApp Backup and Recovery のポリシーの違いを理解してください。
...
NetApp Backup and Recovery では、以下のスケジュール階層が使用されますが、それぞれ若干異なります：
...
NetApp Backup and Recovery は、SnapCenter スケジュールを次のように調整します：
...
* SnapCenterのスケジュールが7日より長く設定されている場合、NetApp Backup and Recoveryはスケジュールを月次に設定します。その月の一部のスナップショットはスキップされます。
+
...
*Example*：1：00 AMから5時間ごとの繰り返し間隔を持つSnapCenterの毎時ポリシーがある場合、NetApp Backup and Recovery（移行後）は、1：00 AM、6：00 AM、11：00 AM、4：00 PM、および9：00 PMの5時間間隔で毎日スケジュールを設定します。一部の時間はスキップされます。9：00 PMの後は、5時間ごとに繰り返すために2：00 AMになるはずですが、常に1：00 AMになります。
