...
---
= カスタムリソースを使用してKubernetesアプリケーションを復元する
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
カスタムリソースを使用して、スナップショットまたはバックアップからアプリケーションを復元します。既存のスナップショットから復元する場合、アプリケーションを同じクラスタに復元する方が通常は高速です。
...
* 詳細設定を使用して復元操作をカスタマイズできます。詳細については、link:br-use-kubernetes-advanced-restore-settings.html["Kubernetes の高度なリストア設定（NetApp Backup and Recovery）"]を参照してください。
...
* CRを使用して新しい名前空間に復元する場合、CRを適用する前に、宛先の名前空間を手動で作成する必要があります。Backup and Recoveryは、CLIを使用した場合にのみ、自動的に名前空間を作成します。
...
NOTE: BackupRestore CR が利用できない場合は、手順 2 のコマンドを使用してバックアップの内容を表示できます。
...
NOTE: ローカルスナップショットからのVMベースのアプリケーションのインプレース復元（元の名前空間および元のクラスタへの復元）は、現在サポートされていません。
