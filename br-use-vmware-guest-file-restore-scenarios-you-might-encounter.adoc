...
---
= NetApp Backup and RecoveryにおけるVMwareゲスト ファイル リストアのトラブルシューティング
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
ゲストファイルの復元中に問題が発生した場合は、以下のシナリオを参考に問題を特定して修正してください。
...
この問題は、ゲストOSのファイル復元操作を開始した際に、VMware Toolsが実行されており、ゲストOSの認証情報が正しいにもかかわらず、ディスクのアタッチ操作が失敗した場合に発生します。この場合、以下のエラーが返されます：
...
この問題を解決するには、ゲストOS上でVMware Tools Windowsサービスを再起動し、ゲストファイルの復元操作を再試行してください。
