...
---
= NetApp Backup and Recoveryを使用してCloud Volumes ONTAPをGoogle Cloud Storageにバックアップする
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
NetApp Backup and Recovery を使用して、Cloud Volumes ONTAP システムのボリューム データを Google Cloud Storage にバックアップします。
...
バックアップを保存するストレージスペースには、Google Cloudアカウントも必要です。
...
NetApp Backup and Recovery の検索と復元機能を使用するには、コンソール エージェントのロールに、Google Cloud BigQuery サービスにアクセスするための特定の権限が含まれている必要があります。以下の権限を確認し、ポリシーを変更する必要がある場合は、手順に従ってください。
...
. *詳細と資格情報*: 次の情報を入力します。
+
.. 「Edit Project」を選択し、使用するプロジェクトがデフォルトのプロジェクト（Console エージェントが配置されているプロジェクト）と異なる場合は、新しいプロジェクトを選択します。
.. クラスタ名を指定します。
...
. *サービス*：NetApp Backup and Recoveryを有効にしたまま、*続行*を選択します。
...
.手順
. コンソールの「Systems」ページからシステムを選択し、右側のパネルにある「Backup and Recovery」の横にある「Enable」を選択します。
+
...
.. 「サービスアカウントのアクセスキー」で「サービスアカウントのキーを作成する」を選択し、先ほど作成したサービスアカウントを選択して、「キーを作成する」を選択します。
+
...
+
バックアップの Google Cloud Storage 保存先がコンソールの *システム* ページにシステムとして存在する場合は、ONTAP クラスターを Google Cloud Storage システムにドラッグできます。
...
** オブジェクトへのバックアップ ポリシーについては、Datalock と Ransomware protection を設定してください。Datalock と Ransomware protection の詳細については、link:prev-ontap-policy-object-options.html["オブジェクトへのバックアップポリシー設定"]を参照してください。
** *作成*を選択します。
...
+
お客様自身が管理するキーを使用する場合は、キーリングとキーの情報を入力してください。
