...
---
= NetApp Backup and RecoveryにおけるONTAPボリューム保護の計画
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
このページを使用して、NetApp Backup and Recovery で ONTAP ボリュームを保護する方法を計画してください。バックアップとリカバリを有効にする前に、保護方法、アーキテクチャ、ポリシー、オブジェクト ストレージの選択肢、および Console エージェントの展開モードを確認してください。
...
使用する機能を選択する前に、各機能の概要と提供される保護の種類について簡単に説明します。
...
| Snapshot | ソース ボリューム内のボリュームの読み取り専用の特定時点のイメージをスナップショットとして作成します。スナップショットを使用して、個々のファイルを回復したり、ボリュームの内容全体を復元したりできます。 
...
|===
スナップショットはすべてのバックアップ方法の基礎であり、バックアップおよびリカバリサービスを利用するために必須です。スナップショットは、読み取り専用のポイントインタイムなボリュームのイメージです。このイメージは最小限のストレージ容量しか消費せず、前回のスナップショット作成以降のファイルの変更点のみを記録するため、パフォーマンス オーバーヘッドはほとんど発生しません。ボリューム上に作成されるスナップショットは、図に示すように、複製されたボリュームとバックアップファイルをソースボリュームに加えられた変更と同期させるために使用されます。

image:diagram-321-overview.png["バックアップ ファイルが、ソース システムではスナップショットとして、セカンダリ ストレージ システムでは複製されたボリュームとして、オブジェクト ストレージではバックアップ ファイルとしてどのように存在するかを示す図。"]
...
これらの基準に加えて、クラウドストレージでは、*DataLockとランサムウェア対策*機能を使用する場合に追加のセキュリティオプションが提供され、古いバックアップファイル用にアーカイブストレージクラスを選択した場合、追加のコスト削減効果が得られます。link:prev-ontap-policy-object-options.html["DataLockとランサムウェア対策およびアーカイブストレージ設定の詳細"]。
...
image:diagram-321-fanout-detailed.png["ボリュームのスナップショットを使用して、複製されたボリュームとバックアップ ファイルを作成および更新する方法を示す図。"]

*カスケード* アーキテクチャでは、まずスナップショットが宛先ストレージ システムに転送され、次にそのシステムがそのコピーをクラウド内のバックアップ オブジェクトに転送します。

image:diagram-321-cascade-detailed.png["ボリュームのスナップショットを使用して、複製されたボリュームとバックアップ ファイルを作成および更新する方法を示す図。"]
...
== スナップショット、レプリケーション、バックアップにはデフォルトのポリシーを使用しますか？

NetAppが提供するデフォルトのポリシーを使用してバックアップを作成するか、カスタムポリシーを作成することもできます。ボリュームのバックアップとリカバリ サービスを有効にするためにアクティベーション ウィザードを使用する場合、デフォルトのポリシーとシステムに既に存在するその他のポリシーから選択できます（Cloud Volumes ONTAPまたはオンプレミス ONTAPシステム）。既存のポリシーとは異なるポリシーを使用したい場合は、アクティベーションウィザードを開始する前、または使用中にポリシーを作成できます。

* デフォルトのスナップショット ポリシーでは、時間別、日次、週次スナップショットが作成され、時間別スナップショットが 6 つ、日次スナップショットが 2 つ、週次スナップショットが 2 つ保持されます。
* デフォルトのレプリケーション ポリシーでは、毎日および毎週のスナップショットがレプリケートされ、7 つの毎日のスナップショットと 52 の毎週のスナップショットが保持されます。
* デフォルトのバックアップ ポリシーでは、毎日および毎週のスナップショットが複製され、7 つの毎日のスナップショットと 52 の毎週のスナップショットが保持されます。
...
スナップショット、レプリケーション、およびオブジェクトストレージへのバックアップ ポリシーは、NetApp Backup and Recovery UIで作成できます。詳細については、link:prev-ontap-backup-manage.html["新しいバックアップポリシーの追加"]を参照してください。

カスタムポリシーを作成するには、NetApp Backup and Recoveryを使用するだけでなく、System ManagerやONTAPコマンドラインインターフェイス（CLI）を使用することもできます：
...
バックアップ ポリシーは、ファンアウトまたはカスケード アーキテクチャのどちらを使用するかによって、異なる場所に保存されます。レプリケーション ポリシーとバックアップ ポリシーは設計が異なります。これは、レプリケーションが 2 つの ONTAP ストレージ システムをペアリングするのに対し、オブジェクトへのバックアップはストレージ プロバイダを宛先として使用するためです。
...
|===
カスケードアーキテクチャを使用する際にカスタムポリシーを作成する予定がある場合は、レプリケートされたボリュームが作成されるセカンダリ システムで、レプリケーション ポリシーとオブジェクトへのバックアップ ポリシーを作成する必要があります。ファンアウト アーキテクチャを使用する際にカスタムポリシーを作成する予定がある場合は、レプリケートされたボリュームが作成されるセカンダリ システムでレプリケーション ポリシーを作成し、プライマリ システムでオブジェクトへのバックアップ ポリシーを作成する必要があります。
...
システムのオブジェクトストレージにバックアップファイルを作成すると、デフォルトでは、バックアップおよびリカバリサービスは、構成済みのオブジェクトストレージアカウント内にバックアップファイル用のコンテナ（バケットまたはストレージアカウント）を作成します。AWS または GCP バケットの名前は、デフォルトでは「netapp-backup-<uuid>」です。Azure Blob ストレージアカウントの名前は「netappbackup<uuid>」です。
...
NOTE: Azureでバックアップ先として使用するコンテナを作成する場合は、作成するコンテナの名前がAzureストレージアカウントの名前と同じであることを確認してください。
...
既にコンソールを使用してストレージを管理している場合は、コンソールエージェントが既にインストールされています。同じコンソールエージェントを NetApp Backup and Recovery で使用する予定がある場合は、準備は万端です。別のコンソールエージェントを使用する必要がある場合は、バックアップとリカバリの実装を開始する前にインストールしてください。
...
完全なインターネット接続環境（「標準モード」または「SaaSモード」とも呼ばれます）でNetApp Backup and Recoveryを使用する場合、Consoleで管理されている任意のオンプレミスONTAPシステムまたはCloud Volumes ONTAPシステム上にレプリケートされたボリュームを作成でき、サポートされている任意のクラウドプロバイダのオブジェクトストレージにバックアップファイルを作成できます。 link:concept-backup-to-cloud.html["サポートされているバックアップ先の完全なリストを参照してください"] 
...
* ローカルのオンプレミス ONTAP システムからローカルの StorageGRID システムにデータをバックアップできます。link:prev-ontap-backup-onprem-storagegrid.html["オンプレミスのONTAPデータをStorageGRIDにバックアップする"]。
* ローカルのオンプレミスONTAPシステムから、ローカルのオンプレミスONTAPシステムまたは S3 オブジェクト ストレージ用に構成されたCloud Volumes ONTAPシステムにデータをバックアップできます。link:prev-ontap-backup-onprem-ontaps3.html["オンプレミスのONTAPデータをONTAP S3 にバックアップする"]。
