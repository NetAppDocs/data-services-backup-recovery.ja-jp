...
SnapCenter にはリソースをホストにマッピングするためのストレージクラスタやストレージ VM はありませんが、NetApp Backup and Recovery にはその機能があります。オンプレミス ONTAP クラスターまたはストレージ VM は、NetApp Backup and Recovery のプレビュー版ではホストにマッピングされません。また、NetApp Console はストレージ VM をサポートしていません。
