...
---
= NetApp Backup and Recovery for ONTAP ボリュームについて
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: 「オンプレミス ONTAP システム」には、FAS、AFF、ONTAP Select の各システムが含まれます。
...
* ボリュームデータのリストアプロセス中にランサムウェアのバックアップファイルをスキャンする場合（クラウドバックアップでDatalockとランサムウェア保護を有効にしている場合）、クラウド プロバイダからの追加のegress コストも発生します。
...
クラスタで ONTAP 9.11.1 以降を使用している場合は、オブジェクト ストレージ内のバックアップを削除やランサムウェア攻撃から保護できます。各バックアップ ポリシーには、特定の期間（_保持期間_）にバックアップ ファイルに適用できる _Datalock とランサムウェアの保護_ のセクションが用意されています。
