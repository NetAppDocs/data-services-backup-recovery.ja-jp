...
---
= NetApp Backup and Recoveryを使用したONTAPボリューム保護について説明します
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
NetApp Backup and Recoveryを使用して、ONTAPボリューム データを保護およびリストアし、バックアップを長期アーカイブします。2つの異なるストレージ システム上のソース データの3つのコピーと、クラウド上の1つのコピーを使用して、3-2-1戦略を実装できます。


NOTE: NetApp Backup and Recoveryのワークロードを切り替えるには、link:br-start-switch-ui.html["さまざまなNetApp Backup and Recoveryワークロードに切り替える"] 。

有効化後、Backup and Recoveryはブロックレベルの増分バックアップを永続的に作成し、別のONTAPクラスターおよびクラウド上のオブジェクトストレージに保存します。ソースボリュームに加えて、以下のものも利用できます：
...
NetApp Backup and Recovery は、NetApp の SnapMirror データレプリケーション テクノロジを使用して、スナップショットを作成し、バックアップ先に転送することで、バックアップの同期を維持します。
...
* 2つのシステム間で事前共有鍵（PSK）を使用して、ネイティブ ONTAP 転送中暗号化を設定します。
* コピーされたデータは、書き込み可能になって使用可能になるまで変更できません。
* 転送に失敗した場合、レプリケーションは自動的に修復されます。
*  https://docs.netapp.com/us-en/data-services-replication/index.html["NetApp Replication"^]と比較すると、NetApp Backup and Recoveryにおけるレプリケーションには次の機能が含まれています：
+
...
* バックアップファイルをスキャンしてランサムウェア攻撃の可能性を検出し、感染したバックアップを自動的に削除または置き換えます。
...
NetApp Backup and Recoveryを使用すると、以下のソースシステムからONTAPボリュームを以下のセカンダリ システムおよびパブリッククラウドとプライベートクラウド プロバイダのオブジェクトストレージにバックアップできます。Snapshotはソースシステムに保存されます。


NOTE: Wasabiは、現時点で唯一の認定されたS3互換オブジェクトストレージプロバイダです。その他のS3互換プロバイダは検証されていません。
...
| S3 互換オブジェクトストレージ | オンプレミスのONTAPシステム | オンプレミスのONTAPシステム 
|===

NOTE: 「オンプレミス ONTAPシステム」とは、FAS、AFF、およびONTAP Selectシステムを指します。
...
* ボリュームデータの復元プロセス中にバックアップファイルをランサムウェアスキャンする予定がある場合（クラウドバックアップに対してDataLockとランサムウェア対策を有効にしている場合）、クラウド プロバイダから追加のデータ転送コストが発生します。
...
NOTE: ボリュームに適用されるスナップショットポリシーには、レプリケーションポリシーおよびオブジェクトへのバックアップポリシーで使用しているラベルのいずれかが含まれている必要があります。一致するラベルが見つからない場合、バックアップファイルは作成されません。例えば、「weekly」のレプリケートされたボリュームとバックアップファイルを作成する場合は、「weekly」スナップショットを作成するスナップショットポリシーを使用する必要があります。
...
クラスターが ONTAP 9.11.1 以降を使用している場合、オブジェクトストレージ内のバックアップを削除やランサムウェア攻撃から保護できます。各バックアップ ポリシーには、特定の期間（保持期間）にわたってバックアップファイルに適用できる「DataLock およびランサムウェア保護」セクションが用意されています。
...
バックアップ先が Amazon S3、Azure Blob、または StorageGRID の場合、現在サポートされています。その他のストレージ プロバイダへの接続先は、今後のリリースで追加される予定です。
...
+
オンプレミスクラスターが ONTAP 9.12.1 以降を使用しており、StorageGRID システムがバージョン 11.4 以降を使用している場合、一定日数経過後に古いバックアップ ファイルをパブリック クラウドのアーカイブ ストレージにアーカイブすることができます。現在サポートされているのは、AWS S3 Glacier/S3 Glacier Deep Archive または Azure Archive ストレージ層です。link:prev-ontap-backup-onprem-storagegrid.html["StorageGRIDからのバックアップファイルのアーカイブについて詳しくは"]。



参照してくださいlink:prev-ontap-policy-object-options.html["古いバックアップファイルのアーカイブの仕組み"]。
