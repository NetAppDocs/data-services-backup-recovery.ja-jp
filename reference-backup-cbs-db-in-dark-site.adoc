...
summary: プライベートモード（インターネットアクセスなし）では、NetApp Backup and Recoveryは、バックアップに使用されるのと同じStorageGRIDまたはONTAP S3バケットに構成データを保存します。コンソールエージェントのホストシステムに障害が発生した場合は、新しいコンソールエージェントをデプロイしてこのデータを復元できます。 
---
...
[role="lead"]
_プライベートモード_（インターネットアクセスなし）では、NetApp Backup and Recoveryは、バックアップに使用されるのと同じStorageGRIDまたはONTAP S3バケットに構成データを保存します。Consoleエージェントのホストシステムに障害が発生した場合、新しいConsoleエージェントを導入して、このデータをリストアできます。

クラウドアカウントが関与しない場合のオンプレミスでの_標準モード_でのバックアップとリカバリの使用時に（たとえば、バックアップ先として NetApp StorageGRID を使用する場合）、以下の手順に従ってコンソールエージェントを交換し、そこに保存されていた構成データを復元することもできます。
...
新しいコンソールエージェントをインストールする際は、元のエージェントと同じソフトウェアバージョンをダウンロードしてください。NetApp Backup and Recoveryデータベースへの変更により、新しいソフトウェアバージョンが古いデータベースバックアップで動作しなくなる可能性があります。 https://docs.netapp.com/us-en/console-setup-admin/task-upgrade-connector.html["バックアップデータベースを復元した後、コンソールエージェントソフトウェアを最新バージョンにアップグレードします。"^]できます。
...
. 先ほど作成した管理者ユーザーの認証情報を使用して、NetApp Console にログインしてください。
...
. MySQL のバックアップを、バックアップ場所から新しい Console エージェントホストにコピーします。次の例では、ファイル名 `CBS_DB_Backup_23_05_2023.sql`を使用します。
...
----
. 表示されるボリュームが元の環境に存在していたボリュームと同じであることを確認します。
...
. インデックス付きカタログのバックアップ ZIP ファイルをバックアップ場所から新しい Console エージェントホスト上の `/opt/application/netapp/cbs`フォルダにコピーします。次の例では、ファイル名 `Indexed_Catalog_DB_Backup_catalogdb1_23_05_2023.zip`を使用します。
...
----
. *ls* コマンドを実行して、「catalogdb1」フォルダが作成され、そのフォルダ内にサブフォルダ `files`、 `versions`、および `snapshots`が含まれていることを確認してください。
...
お客様の ONTAP システムに関連付けられている StorageGRID システムの詳細を、元の Console エージェント設定で構成されたとおりに、 https://docs.netapp.com/us-en/console-automation/index.html["NetApp ConsoleAPI"^]を使用して追加します。
...
----
+
IPアドレス、ユーザー名、パスワードの値はカスタム値ですが、アカウント名はカスタム値ではありません。アカウント名は常に `account-DARKSITE1`です。また、ユーザー名はメールアドレス形式の名前を使用する必要があります。

+
このAPIは、以下のようなレスポンスを返します。認証トークンは以下のように取得できます。
...
----
+
このAPIは、以下のようなレスポンスを返します。 `resourceIdentifier`の値は _WorkingEnvironment ID_ を表し、 `agentId`の値は _x-agent-id_ を表します。
...
----
. システムに関連付けられている StorageGRID システムの詳細を使用して、NetApp Backup and Recovery データベースを更新します。以下に示すように、StorageGRID の完全修飾ドメイン名、アクセスキー、およびストレージキーを入力してください：
+
[source, shell]
----
...
. 各 ONTAP システムを選択し、右側のパネルにある Backup and Recovery サービスの横にある「バックアップの表示」を選択します。
+
ボリュームに対して作成されたすべてのバックアップが表示されます。

. 復元ダッシュボードの「検索と復元」セクションで、*インデックス設定*を選択します。
+
以前インデックス付きカタログ機能が有効になっていたシステムは、引き続き有効になっていることを確認してください。
