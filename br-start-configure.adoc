...
* <<ホストリソースの資格情報を追加する>>Windows、Microsoft SQL Server、Oracle Database、またはLinuxホストでNetApp Backup and Recoveryが認証する必要があるもの。これには、ファイルやフォルダをリストアする際に使用されるゲストOSのクレデンシャルが含まれます。
...
リソースを復元する際に行う一般的な変換を保存するために、リソース変換テンプレートを作成します。

.手順
. NetApp Backup and Recoveryメニューから、[設定] を選択します。
. 下向き矢印を選択して、*Resource modifer template* セクションを展開します。
. *テンプレートを作成*を選択します。
...
.. 必要に応じて、リソースグループ、バージョン、名前を入力します。
