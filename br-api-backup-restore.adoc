...
.手順
. NetApp Console の Web サイトからユーザートークンを取得します。
+
以下のリンクから必ずリフレッシュトークンを生成してください：https://services.cloud.netapp.com/refresh-token/.リフレッシュトークンは、ユーザートークンを生成するために使用する英数字の文字列です。
