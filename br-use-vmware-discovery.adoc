...
[role="lead"]
ONTAP システムで実行されている VMware データストアと VM を保護する前に、NetApp Backup and Recovery でそれらを検出する必要があります。すでに SnapCenter Plug-in for VMware vSphere がインストールされている場合は、そのバックアップデータとポリシーをインポートすることもできます。
...
まだ SnapCenter Plug-in for VMware vSphere がインストールされていない場合は、NetApp Backup and Recovery がワークロードを検出できるように vCenter の情報を追加します。各 Console エージェント内で、ワークロードを検出するシステムを選択します。
...
すでに SnapCenter Plug-in for VMware がインストールされている場合は、次の手順に従って SnapCenter Plug-in のリソースを NetApp Backup and Recovery にインポートします。NetApp Console は、プラグインから ESXi ホスト、データストア、VM、およびスケジュールを検出するため、これらの情報を再作成する必要はありません。
...
NOTE: SnapCenter プラグインがすでに登録されている場合は、既存の登録情報を更新できます。
