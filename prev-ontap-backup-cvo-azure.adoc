...
---
= NetApp Backup and Recoveryを使用してCloud Volumes ONTAPをAzure Blobストレージにバックアップする
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
NetApp Backup and Recoveryを使用して、お客様のCloud Volumes ONTAPシステムからAzure Blobストレージにボリューム データをバックアップします。
...
バックアップを保存するストレージスペースには、Azureサブスクリプションも必要です。
...
NetApp Backup and Recovery の検索および復元機能を使用するには、コンソールエージェントのロールに、Azure Synapse ワークスペースと Azure Data Lake Storage アカウントにアクセスするための特定の権限が含まれている必要があります。以下の権限を確認し、ポリシーを変更する必要がある場合は手順に従ってください。
...
保護するボリュームを選択してください。保護ボリュームとは、スナップショットポリシー、レプリケーションポリシー、オブジェクトへのバックアップポリシーのうち、1つ以上が設定されているボリュームのことです。
...
NOTE: 既存のAzureストレージアカウントを選択した場合、暗号化情報は既に用意されているため、ここで入力する必要はありません。
