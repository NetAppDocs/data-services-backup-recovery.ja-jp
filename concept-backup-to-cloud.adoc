...
サービス料金はNetAppに支払われ、オブジェクト ストレージまたはONTAPターゲットへのバックアップを作成するコストと、それらのバックアップからボリュームまたはファイルを復元するコストの両方をカバーします。オブジェクト ストレージまたはONTAP ( SnapMirror構成を含む) で保護するデータに対してのみ料金をお支払いいただきます。料金は、オブジェクト ストレージまたはONTAPターゲットにバックアップされるONTAPボリュームのソース論理使用容量 ( ONTAP効率前) を使用して計算されます。この容量は、フロントエンド テラバイト (FETB) とも呼ばれます。
