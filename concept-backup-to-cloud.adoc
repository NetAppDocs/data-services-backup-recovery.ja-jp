...
[role="lead"]
この概要では、NetApp Backup and Recovery が ONTAP ワークロードをどのように保護するか、サポートするワークロードとバックアップ対象、およびライセンスとコストの仕組みについて説明します。
...
バックアップとリカバリのサポートは既にすべての ONTAP システムに組み込まれているため、追加のハードウェアやメディアゲートウェイは不要です。これにより、バックアップ作業が簡素化され、コスト効率も向上します。NetApp Console は、複数のリソースマネージャーや専門の担当者を必要とせずに、3-2-1 バックアップの全種類を含むあらゆるバックアップ戦略の実装を簡素化します。
...
** VMwareワークロードを、サポートされているストレージプロバイダーにバックアップします。
...
** Google Cloud では、新しいバケットがデプロイされ、 https://cloud.google.com/bigquery["Google Cloud BigQuery サービス"^]はアカウント/プロジェクトレベルでプロビジョニングされます。


* アーカイブ オブジェクト ストレージに移動されたバックアップ ファイルからボリューム データを復元する場合は、クラウド プロバイダーから追加の GiB あたりの取得料金とリクエストあたりの料金が発生します。
* ボリュームデータのリストアプロセス中にランサムウェアのバックアップファイルをスキャンする場合（クラウドバックアップでDatalockとランサムウェア対策を有効にしている場合）、クラウド プロバイダからの追加のegress コストも発生します。
...
* 3つ目の選択肢は、NetAppから直接ライセンスを購入することです。詳細については、<<ライセンス>>セクションを参照してください。
...
* Google Cloud Storage （ONTAPボリュームとKubernetesワークロードのみ）
* StorageGRID
* ONTAP S3
* S3互換ストレージ（ONTAP Volumesワークロードのみ）
...
* Azureでは、NetApp Consoleは、Blobコンテナ用のストレージアカウントを持つ新規または既存のリソースグループを使用します。Consoleは https://docs.microsoft.com/en-us/azure/storage/blobs/anonymous-read-access-prevent["BLOBデータへのパブリックアクセスをブロックします"]デフォルトで使用されます。
...
== NetApp Backup and Recovery の用語
