...
summary: カスタムリソース（CR）を使用して、Kubernetesアプリケーションのオンデマンドスナップショットとバックアップを作成し、ベースラインを確立したり、最新のデータ変更を保護したりします。 
---
= NetApp Backup and Recoveryのカスタムリソースを使用して、Kubernetesアプリケーションを今すぐバックアップします
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
カスタムリソース（CR）を使用して、Kubernetesアプリケーションのオンデマンドスナップショットとバックアップを作成し、ベースラインを確立したり、最新のデータ変更を保護したりします。
...
** *spec.reclaimPolicy*: （_オプション_） スナップショット CR が削除されたときにAppArchiveに何が起こるかを定義します。 `Retain`に設定した場合でも、スナップショット CR 自体は削除されます。AppArchiveのみが保持されます。有効なオプション：
+
*** `Retain`（デフォルト）
*** `Delete`
+
...
** *spec.dataMover*：（_オプション、Trident Protectから移行したクラスタの場合は必須_）バックアップ処理に使用するバックアップツールを示す文字列。このクラスタがTrident ProtectからBackup and Recoveryに移行されている場合、値は大文字と小文字が区別され、 `CBS`である必要があります。
...
| protect.trident.netapp.io/pvc-bind-timeout-sec | string | 新しく作成された PersistentVolumeClaims （PVC）が操作失敗前に `Bound`フェーズに到達するまでの最大待機時間（秒単位）。 | 「1200」（20分） 
|===