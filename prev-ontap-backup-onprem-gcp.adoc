...
[role="lead"]
NetApp Backup and Recoveryでは、オンプレミスのONTAPシステムからセカンダリ システムおよびGoogle Cloud Storageにボリューム データをバックアップできます。このページを使用して環境を準備し、バックアップを有効にします。
...
* デフォルトとは異なるIPspaceを使用している場合は、オブジェクトストレージにアクセスするために静的ルートを作成する必要がある場合があることに注意してください。
...
+
バックアップ用の Google Cloud Storage の保存先がコンソールの *システム* ページに存在する場合は、ONTAP クラスターを Google Cloud Storage にドラッグできます。
...
** *暗号化キー*：新しい Google Cloud Storage バケットを作成した場合は、プロバイダから提供された暗号化キー情報を入力してください。データの暗号化を管理するために、デフォルトの Google Cloud 暗号化キーを使用するか、Google Cloud アカウントから独自のお客様管理キーを選択するかを選択してください。
+

NOTE: 既存のGoogle Cloud Storageバケットを選択した場合、暗号化情報は既に用意されているため、ここで入力する必要はありません。
