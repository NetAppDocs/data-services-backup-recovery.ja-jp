...
*** *その他の Backup and Recovery のロール*：Backup and Recovery ビューアーや Backup and Recovery クローン管理者など、ユーザーに必要な特定の Backup and Recovery のロールを割り当てます。これらのロールにより、ユーザーはリソースの表示、ワークロードの保護、クローン作成、リストアなどの日常的なタスクを実行できます。Backup and Recovery のロールは、プラットフォームレベルのロールとは別に割り当ててください。詳細については、link:https://docs.netapp.com/us-en/console-setup-admin/reference-iam-backup-rec-roles.html["NetApp ConsoleのNetApp Backup and Recoveryの役割"^]を参照してください。
...
* link:br-use-manage-kubernetes-access-control.html["Kubernetesリソースへのアクセスを管理する"]
