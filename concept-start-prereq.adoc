...
| 必要なソフトウェア パッケージ  a| 
* ASP.NET Core Runtime 8.0.23 Hosting Bundle（およびそれ以降のすべての 8.0.x パッチ）
* PowerShell 7.4.13
...
| ASP.NET Core ランタイム | 8.0.23 Hosting Bundle（およびそれ以降のすべての 8.0.x パッチ） 
...
* ONTAP 9.13.1 以降
* Amazon FSx for NetApp ONTAP バージョン 9.13.1 以降を実行しているストレージ システム
* SANストレージ（プレビュー版）（プライマリストレージおよびセカンダリストレージからのバックアップと元の場所への復元に対応）
